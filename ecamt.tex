\input{header}
\title{Elliptic curves and Mordell's theorem}
\subtitle{2022 Xena Project Undergraduate Workshop}
...
\begin{frame}{Integer solutions}

Consider Mordell's equation
$$ y^2 = x^3 + k, \qquad k \in \Z. $$
What are the integer solutions?

\begin{minipage}{0.6\textwidth}
$$
\begin{array}{c|c}
k & \#\{(x, y) \in \Z^2 : y^2 = x^3 + k\} \\
\hline
-24 & 0 \\
-6 & 0 \\
-5 & 0 \\
...
\item $ k = 16 $: $ (0, \pm 4) $ \\ (use UF of $ \Z $)
\item $ k = -1 $: $ (1, 0) $ \\ (use UF of $ \Z[i] $)
\end{itemize}
\end{minipage}

\bigskip \emph{Siegel's theorem} says that there are only finitely many integer solutions.

\end{frame}

\begin{frame}{Rational solutions}
...
$$
\begin{array}{c|c|c}
k & \#\{(x, y) \in \Z^2 : y^2 = x^3 + k\} & \#\{(x, y) \in \Q^2 : y^2 = x^3 + k\} \\
\hline
-24 & 0 & 0 \\
-6 & 0 & 0 \\
-5 & 0 & 0 \\
...
More generally, an \textbf{elliptic curve} over a field $ F $ is a pair $ (E, \OO) $ of
\begin{itemize}
\item a smooth projective curve $ E $ of genus one defined over $ F $, and
\item a distinguished point $ \OO $ on $ E $ defined over $ F $.
\end{itemize}
...
\bigskip With this definition, an elliptic curve over $ F $ is precisely the data of the five coefficients $ a_1, a_2, a_3, a_4, a_6 \in F $ and a proof that $ \Delta \ne 0 $.
...
\begin{frame}[fragile]{$ K $-rational points}
...
\bigskip Thus, a $ K $-rational point on $ E $ is either
\begin{itemize}
\item the unique point at infinity, or
...
\bigskip Group operations are characterised by
...
\begin{frame}[fragile]{Identity and negation}

More importantly, $ E(K) $ can be endowed with a group structure.

\bigskip Identity is trivial.
...
\bigskip Addition is complicated.
...
\begin{frame}[fragile]{Group axioms}

More importantly, $ E(K) $ can be endowed with a group structure.

\bigskip The remaining group axioms are doable except associativity.
...
\begin{frame}[fragile]{Functoriality and Galois module structure}

Modulo associativity, what basic properties can be stated or proven?

\bigskip Functoriality from field extensions to abelian groups.
...
\begin{frame}[fragile]{Isomorphism of elliptic curves}

Modulo associativity, what basic properties can be stated or proven?

\bigskip Isomorphism given by an admissible change of variables.
...
\begin{frame}[fragile]{$ 2 $-division polynomial and $ 2 $-torsion subgroup}

Modulo associativity, what basic properties can be stated or proven?

\bigskip Polynomial determining points in the $ 2 $-torsion subgroup.
...
\begin{frame}[fragile]{Mordell's theorem}
...
As a consequence of the structure theorem, $ E(\Q) $ can be written as the product of a finite group and a finite number of copies of $ \Z $.
...
Three steps.
\begin{itemize}
\item \textbf{Weak Mordell}: $ E(\Q) / 2E(\Q) $ is finite.
\item \textbf{Heights}: $ E(\Q) $ can be endowed with a ``height function''.
...
\begin{frame}{Weak Mordell}

\begin{proof}[Proof that $ E(\Q) / 2E(\Q) $ is finite.]
\renewcommand\qedsymbol{}
\begin{itemize}
...
\item Prove $ \im(\delta) \subseteq K(S, 2) $ by a simple $ p $-adic analysis.
\item Prove $ K(S, 2) $ is finite by classical algebraic number theory. $ \square $
\end{itemize}
\end{proof}
...
\bigskip The finiteness of $ K(S, n) $ reduces to the finiteness of $ K(\emptyset, n) $, which boils down to two fundamental results in classical algebraic number theory.
\begin{itemize}
\item The \emph{class group} $ \Cl_K $ is finite.
\item The \emph{unit group} $ \OO_K^\times $ is finitely generated.
\end{itemize}

\bigskip Then $ K(\emptyset, n) $ can be nested in a short exact sequence
$$ 0 \to \OO_K^\times / (\OO_K^\times)^n \to K(\emptyset, n) \to \Cl_K[n] \to 0, $$
whose flanking groups are both finite, so $ K(\emptyset, n) $ is also finite.

\end{frame}

\begin{frame}{Heights and descent}

\begin{proof}[Proof that $ E(\Q) / 2E(\Q) $ finite implies $ E(\Q) $ finitely generated.]
There is a function $ h : E(\Q) \to \R $ with the following three properties.
\begin{itemize}
\item For all $ Q \in E(\Q) $, there exists $ C_1 \in \R $ such that for all $ P \in E(\Q) $,
$$ h(P + Q) \le 2h(P) + C_1. $$
\item There exists $ C_2 \in \R $ such that for all $ P \in E(\Q) $,
$$ 4h(P) \le h(2P) + C_2. $$
