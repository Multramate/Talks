\input{header}
\title{Elliptic curves in Lean}
\subtitle{Mathematical Theorem Proving Workshop \\ \includegraphics[width=0.2\textwidth]{img/huawei.png}}
...
\bigskip Public-key cryptography (over a large finite field)
\begin{itemize}
\item Integer factorisation (\emph{e.g.} Lenstra's method) \\
Breaks the RSA cryptosystem
\item Diffie--Hellman key exchange \\
Discrete logarithm (solve $ nQ = P $ given $ P $ and $ Q $)
\item Supersingular isogeny Diffie--Hellman key exchange
\end{itemize}

\bigskip Number theory (over a field/ring/scheme)
\begin{itemize}
\item The simplest non-trivial objects in algebraic geometry
\item Rational elliptic curve associated to $ a^p + b^p = c^p $ cannot be modular \\
...
\bigskip For a scheme $ T $ over $ S $, define the set of \textbf{$ T $-points} of $ E $ by
$$ E(T) := \Hom_S(T, E), $$
which is naturally identified with a \emph{Picard group} $ \Pic_{E / S}^0(T) $ of $ E $.
...
Let $ T / S $ be a field extension $ K / F $. \footnote{$ S = \Spec(F) $ and $ T = \Spec(K) $, or even rings whose class group has no $ 12 $-torsion}
...
\bigskip The Picard group becomes
...
\begin{frame}{Concretely}

The Riemann--Roch theorem gives \emph{Weierstrass equations}.

\bigskip $ E(K) $ is basically the set of solutions $ (x, y) \in K^2 $ to
$$ y^2 + a_1xy + a_3y = x^3 + a_2x^2 + a_4x + a_6, \qquad a_i \in F. $$
If $ \ch(F) \ne 2, 3 $, can reduce this to
$$ y^2 = x^3 + Ax + B, \qquad A, B \in F. $$
The \emph{group law} is reduced to drawing lines.
\begin{center}
\includegraphics[width=0.4\textwidth]{img/grouplaw.png}
\end{center}
...
\begin{itemize}
\item $ 1. $ and $ 2. $ require much algebraic geometry (properness, genus, ...)
\item $ 2. = 3. $ also requires algebraic geometry (divisors, differentials, ...)
\item $ 3. $ requires just five coefficients (and a non-zero \emph{discriminant})!
\end{itemize}
...
This is the \emph{scheme} $ E $, but what about the \emph{abelian group} $ E(F) $?

\end{frame}

\begin{frame}[fragile]{Points}
...
\bigskip Commutativity is doable
...
\bigskip Associativity is difficult
...
\item Attempt (by M Masdeu) to bash it out using \texttt{linear\_combination}
\item Proved (by E-I Bartzia and P-Y Strub) in Coq \footnote{A Formal Library for Elliptic Curves in the Coq Proof Assistant (2015)} \\
that $ E(K) \cong \Pic_{E / F}^0(K) $ for $ \ch(F) \ne 2, 3 $
\end{itemize}
...
\bigskip Functoriality $ \Alg_F \to \Ab $
...
Galois module structure $ \Gal(L/K) \curvearrowright E(L) $
...
\bigskip Isomorphisms $ (x, y) \mapsto (u^2x + r, u^3y + u^2sx + t) $
...
\bigskip $ 2 $-division polynomial $ \psi_2(x) $
...
\begin{frame}{Mordell--Weil}

Let $ K $ be a number field. Then $ E(K) $ is finitely generated.

\bigskip Show $ E(K)/2E(K) $ is finite:
\begin{enumerate}
\item Reduce to $ E[2] \subset E(K) $
\item Define $ 2 $-descent $ \delta : E(K)/2E(K) \hookrightarrow K^\times / (K^\times)^2 \times K^\times / (K^\times)^2 $
\item Show that $ \im\delta \le K(\emptyset, 2) \times K(\emptyset, 2) $
\item Prove exactness of $ 0 \to \OO_K^\times / (\OO_K^\times)^n \to K(\emptyset, n) \to \Cl_K[n] \to 0 $
\item Apply finiteness of $ \OO_K^\times / (\OO_K^\times)^n $ and $ \Cl_K[n] $
\end{enumerate}

\bigskip Show this implies $ E(K) $ is finitely generated:
\begin{enumerate}
\item Define heights on elliptic curves
\item (J Zhang) Prove the descent theorem
\end{enumerate}

\bigskip Soon: Mordell's theorem for $ E[2] \subset E(\Q) $.
