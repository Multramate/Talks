\def\type{}
\input{header}
\title{An introduction to pairing-based elliptic curve cryptography}
...
\vspace{-1cm}
\begin{center}
\includegraphics[width=0.2\textwidth]{img/adjoint.png}
\end{center}
\vspace{-1cm}
...
It turns out that this definition of an abelian group is indeed well-defined, but in certain edge cases, the inverse of a point or the addition of two points may not exist in the plane, and in these cases they will be defined as the point at infinity $ \OO $. It will be remarked that the group axiom of associativity is by far the most difficult to verify, and several general geometric techniques can been developed just to tackle this, but this is indeed true and can be implicitly assumed in the uses in ECC.
...
$$ E(\F_{p^{12}}) = \{(x, y) \in \F_{p^{12}}^2 : y^2 = x^3 + b\} \cup \{\OO\}, $$
